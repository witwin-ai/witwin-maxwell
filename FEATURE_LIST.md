...
- `Material.orientation` remains unsupported. Full off-diagonal `Tensor3x3` permittivity is FDTD-only and cannot currently overlap PML/absorber cells or combine with Bloch fields, conductivity, Kerr media, or polarized subpixel averaging; it now composes with electric dispersive poles in the same material (isotropic poles over a full-tensor `eps_inf`, a rotated birefringent dispersive crystal), which is FDTD forward-only. Static full-tensor scenes are differentiable through the FDTD adjoint bridge (a design differentiated through the static off-diagonal coupling), but a trainable geometry on the tensor structure itself, anisotropic `mu_tensor` media, and the full-tensor + electric-dispersion combination stay guarded because the off-diagonal coupling, magnetic reverse updates, and coupled-tensor ADE reverse carry no material tensor gradient channel.
