...
- Optional CUDA-graph capture of the field-update core via `Simulation.fdtd(..., cuda_graph=True)`: the magnetic/electric Yee updates (with CPML) are captured once and replayed per step, collapsing per-step kernel-launch overhead. Bit-exact against the non-graph path, opt-in, and automatically declined for TFSF / magnetic-source / complex-field / Kerr / dispersive scenes whose update block carries per-step host input. Most effective in the launch-bound small/adaptive-grid regime (measured ~2x/step on a ~10^5-cell AutoGrid scene, negligible on large compute-bound uniform grids); `Result.stats()["cuda_graph_active"]` reports whether capture engaged
