...
- `Graphene(chemical_potential=..., scattering_time=..., temperature=300.0)` 2D sheet with the Kubo intraband (Drude-like) surface conductivity `sigma_s(omega) = A / (1/tau - i*omega)` (`chemical_potential` in eV, `temperature` in K, `scattering_time` in s): the compiler lowers the sheet current relaxation to an equivalent tangential-only Drude pole on the snapped Yee layer (`eps0*omega_p^2 = A/dcell`), advanced by the existing native CUDA Drude ADE kernels; the intraband relaxation rate feeds the FDTD auto-`dt` bound, and `include_interband=True` raises explicitly
- `LossyMetalMedium(conductivity=...)` good-conductor descriptor with analytic Leontovich helpers `surface_impedance(omega)` / `surface_impedance_at_freq(f)` / `skin_depth(f)`; the surface-impedance boundary-condition runtime is not implemented yet and compiling a scene containing it raises `NotImplementedError` explicitly
...
- `Medium2D` sheets require an axis-aligned unrotated `Box` geometry with exactly one zero-size axis, snap to the nearest node plane along the sheet normal (staircase placement, no sub-cell interpolation), and are rejected explicitly by Tidy3D export and by the FDTD/FDFD adjoint bridges
- `Graphene` implements the Kubo intraband term only; the interband conductivity does not map onto the real-coefficient pole machinery without a Pade/pole fit and `include_interband=True` raises `NotImplementedError` (interband contributions matter for `hbar*omega` approaching `2*|mu_c|`, e.g. optical bands at low doping)
- `LossyMetalMedium` is a descriptor-only feature in v1: the Leontovich SIBC update `E_t = Z_s(omega) * (n x H)` needs a vector-fitted pole expansion of the `sqrt(-i*omega)` impedance with per-face recursive-convolution state and a boundary-side kernel, so scene compilation and Tidy3D export both raise `NotImplementedError`; resolve such metals volumetrically with `Material(sigma_e=...)` or `Material.pec()`
