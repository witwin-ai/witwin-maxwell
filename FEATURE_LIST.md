...
- Instantaneous second-order nonlinearity through `NonlinearSusceptibility(chi2=...)` composed via `Material(nonlinearity=...)` (single descriptor or a tuple): per component `P_i = eps0 * chi2 * E_i^2`, which drives second-harmonic generation and optical rectification (FDTD only); multiple descriptors sum, and nonlinear media may now coexist with dispersive or anisotropic materials in other structures of the same scene
