...
- Space-time modulation (`ModulationSpec`) is FDTD-only and modulates the electric permittivity only (no `mu`/`sigma_e` modulation); a modulated `Material` cannot carry dispersive poles, anisotropic tensors, nonlinear channels, or `sigma_e`, and modulated scenes cannot mix with dispersive, nonlinear, or fully anisotropic structures elsewhere in the same scene. A scene supports a single modulation frequency, grid-valued `amplitude`/`phase` require `Box` structure geometry, the modulation depth is capped below `0.5` for Courant stability, Bloch / complex-field runs are rejected, CUDA-graph capture is disabled (per-step host phase scalars), and the FDTD adjoint bridge and Tidy3D export reject modulated media explicitly. `GridSpec.auto` and static/frequency evaluation use the unmodulated carrier permittivity `eps_static`
