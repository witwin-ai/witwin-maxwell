...
- `Material.orientation` remains unsupported. Full off-diagonal `Tensor3x3` permittivity is FDTD-only and cannot currently overlap PML/absorber cells or combine with Bloch fields, conductivity, dispersion, Kerr media, or polarized subpixel averaging; it is differentiable through the FDTD adjoint bridge (a design differentiated through the static off-diagonal coupling), except that a trainable geometry on the tensor structure itself and anisotropic `mu_tensor` media stay guarded because the off-diagonal coupling and magnetic reverse updates carry no material tensor gradient channel.
