...
- Tidy3D export rejects anisotropy, magnetic dispersion, and Kerr media explicitly in v1. The FDTD adjoint bridge supports diagonal (`DiagonalTensor3`) epsilon anisotropy and scenes containing static magnetic-dispersive (mu-pole) media, but still rejects full off-diagonal (`Tensor3x3`) permittivity, `mu_tensor` anisotropy, trainable geometry on magnetic-dispersive structures (there is no mu material-gradient channel), and Kerr media explicitly
