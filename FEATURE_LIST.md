...
- `UniformCurrentSource`, `CustomCurrentSource`, and `CustomFieldSource` are v1 forward-only: the FDTD adjoint bridge rejects them (no source pullback is implemented). `CustomFieldSource` additionally requires a planar (single-sample-axis) `FieldDataset`, injects toward `+normal` only, and reproduces the plane wave to discretization-limited tolerance (finite-aperture edge diffraction and half-cell E/H staggering leave a small backward and cross-polarized residual)
