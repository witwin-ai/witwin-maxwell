...
- A single `Material` cannot combine instantaneous nonlinearity (`kerr_chi3` / `NonlinearSusceptibility` / `TwoPhotonAbsorption`) with dispersive poles or anisotropic tensors; separate structures in the same scene may mix them freely. Nonlinear media are excluded from Bloch / complex-field runs, full off-diagonal anisotropy, CUDA-graph capture, FDFD, and Tidy3D export. The FDTD adjoint bridge covers pure instantaneous Kerr (chi3) media but rejects chi2 and two-photon absorption explicitly. `NonlinearSusceptibility` is scalar (tensorial chi2 / chi3 are not supported)
...
- Tidy3D export rejects anisotropy, magnetic dispersion, and Kerr media explicitly in v1. The FDTD adjoint bridge supports diagonal (`DiagonalTensor3`) epsilon anisotropy, scenes containing static magnetic-dispersive (mu-pole) media, and pure instantaneous Kerr (chi3) media, but still rejects full off-diagonal (`Tensor3x3`) permittivity, `mu_tensor` anisotropy, trainable geometry on magnetic-dispersive structures (there is no mu material-gradient channel), chi2, and two-photon-absorption media explicitly
