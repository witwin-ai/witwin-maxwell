...
def slab_scene(material: mw.Material, *, name: str, thickness: float = 0.1) -> mw.Scene:
    ...
    scene = (
        ...
        .add_monitor(
            mw.FluxMonitor(name="reflected", axis="z", position=-0.3, frequencies=(FREQUENCY,), normal_direction="-")
        )
        .add_monitor(mw.FluxMonitor(name="transmitted", axis="z", position=0.3, frequencies=(FREQUENCY,)))
    )
    return with_plot_monitors(scene, frequencies=(FREQUENCY,))
