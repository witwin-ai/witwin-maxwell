from __future__ import annotations

import witwin.maxwell as mw

from benchmark.models import ScenarioDefinition
from benchmark.scenes._common import base_scene, with_plot_monitors


FREQUENCIES = (1.5e9,)


def build_scene() -> mw.Scene:
    scene = (
        base_scene()
        .add_structure(mw.Structure(geometry=mw.Box(position=(0.0, 0.0, 0.0), size=(0.24, 0.24, 0.24)), material=mw.Material(eps_r=4.0), name="dielectric_box"))
        .add_source(
            mw.PointDipole(
                position=(0.0, 0.0, -0.20),
                polarization="Ez",
                profile="ideal",
                source_time=mw.GaussianPulse(frequency=1.5e9, fwidth=0.5e9),
                name="dipole",
            )
        )
        .add_monitor(mw.PlaneMonitor(name="field_xz", axis="y", position=0.0, fields=("Ex", "Ez", "Hy"), frequencies=FREQUENCIES))
        .add_monitor(mw.FluxMonitor(name="flux_pos_z", axis="z", position=0.30, frequencies=FREQUENCIES))
        .add_monitor(mw.FluxMonitor(name="flux_neg_z", axis="z", position=-0.30, frequencies=FREQUENCIES, normal_direction="-"))
    )
    return with_plot_monitors(scene, frequencies=FREQUENCIES)


SCENARIO = ScenarioDefinition(
    name="dipole_dielectric_box",
    description="Ez dipole coupled to a dielectric box.",
    builder=build_scene,
    frequencies=FREQUENCIES,
    display_monitor="field_xz",
    display_component="Ez",
)
