...
### 2026-07-18 完成记录（Phase 0-3 + Phase 4 权重梯度完成，master `e0a0877`）
...
### 2026-07-21 修订（Phase 4 场景梯度贯穿落地 + 2 GPU 聚合，track F3；append-only）

依无通胀规则追加；上文完成记录与 fail-closed 登记逐条保留存档，本栏只登记 F3a/F3b 的正向更新，如与上文"场景/材料/几何梯度 fail-closed"声明冲突，以本栏为准。

- **F3a（单 GPU 场景梯度贯穿，关闭 fail-closed）**：`ArrayBasisData.scene_gradient_vjp(...)` 已实现聚合 per-column 伴随，替换原 `NotImplementedError`；capability-guard census `176 → 175`（`tests/api/public/test_guard_census.py`，disposition 见 `docs/reference/fdtd-capability-guard-census.md`）。权重共轭约定 `seed_n = conj(w_n)·cot_E`（= PyTorch 复乘 backward），对固定 reduction order 与端到端 `autograd.grad` **逐位相同**；真实两列 FDTD 阵列（可训练 `MaterialRegion` 密度、NF2FF 远场列）的中心差分门在 FDTD 伴随容差内通过。验收：`docs/assessments/f3-array-vjp-acceptance-2026-07-21.md`。上文第 5 类"提升到 E2/E3 所需证据"第 1 项（完成 `scene_gradient_vjp` 聚合 per-column 伴随、关闭 fail-closed）由此结算。**注意**：这里承诺的是远场 VJP 贯穿 basis 列到场景参数；network / S 参数场景梯度不在其内，`combine()` 权重梯度不变。
- **F3b（任务级 2 GPU 聚合）**：新增 `witwin/maxwell/array_gradient.py` 的 `aggregate_scene_gradient_vjp(...)` 与 `ensemble_scene_gradient_vjp(...)`。per-column 前向作为**独立 Simulation** 分发到 ensemble device pool（安全路径；NCCL 联合求解伴随仍不在范围），按公开端口顺序在指定 reduction device 显式组装 VJP sum 并记录 aggregation provenance（对应上文 02 blueprint "在指定 result device 上按 public port order 显式组装 VJP sum，并记录 aggregation provenance"）。**1 GPU 对 2 GPU 聚合梯度 parity 实测为逐位相同**（同构 A6000；合成 float64 图与真实两列 FDTD 阵列各测 maxabsdiff = 0），即 F3a 预判的"`<1e-12` floor 或 bitwise"结算为 **bitwise**。验收：`docs/assessments/f3-array-scene-vjp-acceptance-2026-07-21.md`。
- **仍为欠账**：吞吐/加速比等 timing 契约未在本轮声明（共享 GPU 正确性窗口），记为 deferred-pending-exclusive-window；上文第 5 类第 2、3 项（阵列因子 + 外部参考求解器多场景对照；端口功率链继承 01 S1 wave 级验证）不变。

#### 2026-07-21 supervisor 合并锚记（append-only）

- 上文 F3a/F3b 已由 supervisor 合并至 master，merge `84ae132`。`00-status-and-gaps-2026-07-19.md` 已据此把 06 证据级重标为 **E2**，Phase-4 exit-gate（scene-gradient VJP 贯穿）项结算；该项即 S3 的 06 成员，S3 由此判 PASS（另两成员：03 round-E 交叉验证 + gate-(d) 裁定、04 F1）。
- **未标 `completed`**：仍无非作者评审（审计 §4：wave-level 门 + 独立参考 + 非作者评审），本轮只记录交付与证据级。
