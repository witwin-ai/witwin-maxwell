...
Round-E 进展（2026-07-21，master `8158014`）：S1 keystone landed — the Yee-staggered
transverse mode operator (`145a0f0`) un-BLOCKED `rectangular_waveguide` (committed
wave-level PASS + authorized external-reference cross-check, `26a7247`); the coax-line
rebuilds lifted `lumped_open_short_match` and `series_parallel_rlc` to wave-level and
the FDTD `antenna/half_wave_dipole` benchmark passes the real NF2FF path (`bb7e746`).
...
### 2026-07-21 修订（Round F track F2 三件套，merge `8477acc`；append-only）

已由 supervisor 合并至 master。逐项登记：

- **Interior-PEC masking + 准静态电磁线模引擎（F2a）**：Yee 交错横向算子上对 PEC 占据的 `Eu`/`Ev`/`Ew` 未知量做对称 Dirichlet 消元 + 每导体连通性检查；变系数 Laplace `div(eps grad phi)=0` 电容比引擎 `eps_eff = C/C0` 服务 TEM/准 TEM 线。算子级门（`tests/rf/wave_validation/test_interior_pec_operator.py`，13 passed）：coax `eps_eff` 精确、微带 `3.0920` vs Hammerstad–Jensen `3.0701`（0.71%，门 ≤3%）、差分对 even/odd 区分、隔板半波导 `beta` 0.11%。验收 `docs/assessments/f2a-interior-pec-acceptance-2026-07-21.md`。
- **更正的 TEM 声明（审计整改）**：早前草稿称交错 curl-curl 算子"结构性排除 TEM"——**错误并撤回**。算子携带梯度 TEM 支为精确本征值 `eps·k0²`；是 shipped occupancy 栅格化（阈 0.5，消去导体表面跨接的法向 `E` 采样）把 TEM 从 **masked** 约化算子中移除，属 masking 选择而非结构性质。TEM/准 TEM 因此 **fail-closed 路由**到准静态引擎（`ValueError`，census 不变）。证据 `test_masked_operator_tem_branch_is_a_masking_artifact`（阈 0.75 保留跨接采样即精确恢复 `beta²=56.25`）。masked 算子上的 coax 交叉实现门交 supervisor 决定（切换消元判据会牺牲薄片 masking）。
- **`rf/microstrip_two_port` / `rf/differential_pair` 从 BLOCKED 解锁为生产准 TEM wave 级 benches（F2b）**：非均匀 interior-PEC 准 TEM 模已接入 WavePort 路径（`quasistatic_line_torch`），终止 FDTD 扫频运行，`B=S·A` 抽取良态（cond(A) 1.23 / 1.31），mixed-mode 转换正确（`|Sdd21|≈0.88 ≠ |Scc21|≈0.68`，`|Sdc21|≈0`）。状态记为 **`gap`（实测，不强判）**——残余为绝对 `eps_eff` 精度受分辨率限制（屏蔽 bench ~1.86 vs H–J ~3.27，dx=5mm 约低 24%，归因 aperture-shielding + 基板分辨率，非抽取缺陷；准静态引擎自身单调收敛到 H–J，16 基板格 <1%）。`tests/rf/wave_validation/test_microstrip_diffpair_wave_level.py`（6 passed）。验收 `docs/assessments/f2b-quasistatic-benches-acceptance-2026-07-21.md`。**guided（非 TEM）interior-PEC 分支仍走 legacy 算子**，masked guided 路径仅被 F2a 算子测试覆盖，未进生产。
- **adapter 端口/lumped 源映射 + 四个外部缓存（F2c）**：`WavePort`→参考 `ModeSource`+`ModeMonitor`，`LumpedPort`→`UniformCurrentSource` delta-gap 电流丝；`TerminalPort` 暂不在驱动范围。四个原 `sources=0` 场景（`coax_thru`、`lumped_open_short_match`、`half_wave_dipole`、`patch`）现均可运行导出并各云生成一次（0.025 FlexCredits，task id 见验收 §4；`benchmark/RESULTS.md` 行 `generated`）。`tests/api/adapters/tidy3d/test_port_source_mapping.py`（6 passed）。验收 `docs/assessments/f2-rf-trio-acceptance-2026-07-21.md`。此解决 owner decision point 1（是否资助 adapter 端口/lumped 源映射）。
- **patch 馈电诊断（fail-closed，xfail 未翻）**：galvanic PEC probe via 把馈电电抗降 ~5×，但 patch 仍不谐振（历史频带偏离谐振 + lumped gap 被相邻 PEC 电容短接 + 有限地过小）；matched-broadside `TM010` strict xfail 保持 fail-closed 守卫，reason 串更新为 F2b 诊断；重设计（多次运行天线协同设计）延后。
- **证据级**：`00-status-and-gaps` 已把 01 验证场景集维持 **E2**、并把 microstrip/diff-pair 由 BLOCKED 改记 `gap`；patch broadside 仍 E0–E1（诚实 strict xfail）。
