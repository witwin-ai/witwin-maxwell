...
- Result assembly in `simulation.py` (`_run_fdtd_from_solver`) via `finalize_breakdown_data`; `Result.breakdown_data` / `Result.breakdown_events` accessors in `result.py`.
...
- No-breakdown-material scene: `Result.breakdown_data is None` (zero-cost path preserved).
...
* **Direct-drive** tests prepare a solver (`Simulation.fdtd(...).prepare().solver`,
  ...
  update.
* **Integration** tests run a real FDTD solve and inspect `Result.breakdown_data`.
...
- `test_breakdown_parity.py` (3): below-threshold descriptor six-field
  ...
  breakdown-free scene -> `Result.breakdown_data is None`; triggering control that
...
- **Global boundary-flux energy balance — decision-6 gate, explicitly deferred
  ...
  (`Result.breakdown_data.total_dissipated_energy = 1.95e-2 J`). Inspecting `Result`
