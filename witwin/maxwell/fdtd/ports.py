...
def accumulate_port_observers(solver) -> None:
    ...
    for runtime in getattr(solver, "_port_runtimes", ()):
        ...
        if runtime.lumped is not None and runtime.wire_provider is not None:
            # The wire terminal coordinate is electric-time data, while its
            # branch current is the conjugate half-step current.  Keep that Yee
            # staggering in the DFT; the circuit midpoint voltage is an energy
            # diagnostic and can contain stiff split impulses.
            provider = runtime.wire_provider
            coordinate = _pack_wire_port_coordinate(
                provider,
                fields,
            ...
            voltage = torch.dot(
                coordinate.reshape(-1), provider.voltage_weights
            )
        ...
        runtime.electric_time.add_(solver.dt)
        runtime.magnetic_time.add_(solver.dt)
