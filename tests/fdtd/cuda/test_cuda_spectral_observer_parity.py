...
def test_compiled_cuda_extension_point_and_plane_observers_match_torch_dispatcher(monkeypatch):
    generator = torch.Generator(device="cuda")
    generator.manual_seed(406)
    field = _rand((4, 5, 6), generator)
    point_i = torch.tensor([0, 1, 3], device="cuda", dtype=torch.long)
    point_j = torch.tensor([2, 4, 1], device="cuda", dtype=torch.long)
    point_k = torch.tensor([5, 0, 3], device="cuda", dtype=torch.long)
