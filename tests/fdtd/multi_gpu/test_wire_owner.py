...
def test_distributed_wire_with_mur_boundary_is_rejected():
    # Fail-closed: wire+Mur edge coupling across the x split is undocumented and
    # unverified, so it must be rejected rather than silently running a partial
    # absorbing coupling that happens to pass the (previously PML-only) gate.
    scene = _wire_scene(boundary=mw.BoundarySpec.mur())
    with pytest.raises(NotImplementedError, match="distributed Mur"):
        DistributedFDTD(scene, frequency=_FREQUENCY, parallel=_parallel())
...
def _public_run(scene, *, parallel, devices, time_steps=_TIME_STEPS):
    """Drive a wire scene through the public Simulation.prepare()+run() path."""

    parallel_config = None
    if parallel:
        parallel_config = FDTDParallelConfig(
            devices=devices,
            transport="cuda_p2p",
            overlap=True,
            gather_fields=True,
...
def test_public_prepare_run_parallel_wire_matches_single_gpu(
    cuda_p2p_devices,
    cuda_memory_cleanup,
):
    """Public-path parity smoke.

    The physical gates above construct DistributedFDTD directly; this exercises the
    supported user entrypoint (Simulation.prepare()+run() with a parallel config) on
    a split-crossing wire scene and asserts it reproduces the single-GPU public run
    bitwise, at the same tolerances as the direct-construction bitwise gate.
    """

    single_result = _public_run(
        _straight_crossing_scene(), parallel=False, devices=cuda_p2p_devices
    )
    parallel_result = _public_run(
        _straight_crossing_scene(), parallel=True, devices=cuda_p2p_devices
    )

    single_wire = single_result.monitors["wire_state"]
    parallel_wire = parallel_result.monitors["wire_state"]
    # The wire is genuinely active across the split.
    assert float(parallel_wire.current.abs().max()) > 1.0

    torch.testing.assert_close(
        parallel_wire.current.cpu(), single_wire.current.cpu(), rtol=0.0, atol=0.0
    )
    torch.testing.assert_close(
        parallel_wire.charge.cpu(), single_wire.charge.cpu(), rtol=0.0, atol=0.0
    )

    # Gathered last-step fields must also match the single-GPU public run bitwise.
    assert single_result.fields and parallel_result.fields
    for name, single_field in single_result.fields.items():
        assert name in parallel_result.fields
        torch.testing.assert_close(
            parallel_result.fields[name].cpu(),
            single_field.cpu(),
            rtol=0.0,
            atol=0.0,
        )
...
def test_two_gpu_closed_loop_crossing_split_matches_single_gpu_final_energy(
    cuda_p2p_devices,
    cuda_memory_cleanup,
):
    """Two-GPU vs single-GPU combined-energy parity for a split-crossing loop.

    This gate compares the *final-step* combined field-plus-wire energy of the
    distributed run against the single-GPU run and asserts they agree to a tight
    relative tolerance. It is a single-vs-two-GPU parity check, not a per-step
    energy-conservation proof: the per-step lossless-loop invariant is gated by
    the single-GPU forward test in tests/fdtd/thin_wire/test_thin_wire_forward.
    """
